...
sqlcmd -S %DBServer% -U %User% -P %Password% -Q "if exists (select * From master.dbo.sysdatabases where name='$(OClient)') BACKUP DATABASE [$(OClient)] TO DISK=N'$(sqlbak)\$(OClient).bak' WITH NAME=N'$(OClient)', INIT, FORMAT, CHECKSUM, STOP_ON_ERROR;"
