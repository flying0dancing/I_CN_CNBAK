...
set "NClient="
set sqlreturnflag=
rem set varALIASTYPE=SQL SERVER
IF NOT "%~4"=="" (
	 set NClient=%~4
)
if defined varALIASTYPE ( echo varALIASTYPE=%varALIASTYPE%)
echo DBServer=%DBServer%
rem IF EXIST "%~dp0\setServer_%Flag%.bat" (
   IF EXIST "%~dp0\setServer_%Flag%.bat" (
     call "%~dp0\setServer_%Flag%.bat"
   ) ELSE ( 
     call "%~dp0\setServer_default.bat"
   )
    IF /I "%OP%"=="create" (
...
rem sqlcmd -S %DBServer% -U %User% -P %Password%  -Q "RESTORE FILELISTONLY FROM DISK=N'$(sqlbak)\$(OClient).bak'; RESTORE DATABASE [$(NClient)] FROM  DISK =N'$(sqlbak)\$(OClient).bak' WITH RECOVERY,NOUNLOAD,REPLACE,MOVE N'$(OClient)' TO N'$(sqlstore)\$(NClient).mdf',MOVE N'$(OClient)_log' TO N'$(sqlstore)\$(NClient)_log.ldf';"
rem sqlcmd -S %DBServer% -U %User% -P %Password%  -Q "if exists (select * From master.dbo.sysdatabases where name='$(NClient)') ALTER DATABASE [$(NClient)] MODIFY FILE (NAME=N'$(OClient)', NEWNAME=N'$(NClient)'); if exists (select * From master.dbo.sysdatabases where name='$(NClient)') ALTER DATABASE [$(NClient)] MODIFY FILE (NAME=N'$(OClient)_Log', NEWNAME=N'$(NClient)_Log');"
...
:mdalias
echo sqlcmd -S %DBServer% -U %User% -P %Password% -d "%OClient%" -i "%~dsp0InsertAliases.sql" -v sqltype="%varALIASTYPE%"
sqlcmd -S %DBServer% -U %User% -P %Password% -d "%OClient%" -i "%~dsp0InsertAliases.sql" -v sqltype="%varALIASTYPE%"
GOTO :EOF
